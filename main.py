...
import ase, ase.io, ase.visualize, ase.neighborlist
...
"""
    ...
    nl = ase.neighborlist.build_neighbor_list(atoms,cutoffs=cutoff)
    connmat = nl.get_connectivity_matrix(False)
    n_components, component_list = sparse.csgraph.connected_components(connmat)

    origsymbols = np.array(atoms.get_chemical_symbols())

    ind_selected = []
    for i in range(n_components):
        ind = np.where(component_list == i)[0]

        if (atoms.get_distances(ind[0], ind) != atoms.get_distances(ind[0],ind, mic=True)).any():
            print ("Restarting!")
            return -1
    ...
    counter = 1
    file_name = r'data.polyethylene1'
    flag = True
    while flag:
        if exists(file_name):
            counter += 1
            file_name = r'data.polyethylene{}'.format(counter)
        else:
            flag = False
            ase.io.write(file_name, atoms, format='lammps-data')
...
for i in range(repeats):
    ...
    while generate == -1:
        generate = main(N,L,rot=True)
    else:
        relaxation(generate, 0) #Perform geometry optimisation on the polyethylene chain generated by main.
